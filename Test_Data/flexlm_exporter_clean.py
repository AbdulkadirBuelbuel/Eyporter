...
def load_servers_yaml(path: str) -> List[FlexLMServerTarget]:
    ...
    try:
        if not os.path.isabs(path):
            path = os.path.join(_app_base_dir(), path)
        ...
        for entry in servers_section:
            ...
            if host and port:
                try:
                    ...
                    continue
                if not os.path.isabs(lmutil_path):
                    lmutil_path = os.path.join(_app_base_dir(), lmutil_path)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FlexLM License Server Exporter für Prometheus mit Username-basiertem Location Mapping')
